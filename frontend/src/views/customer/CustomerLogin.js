    ...
    <div className="sf-auth-page" data-testid="customer-login-page">
      <div className="sf-auth-card">
        ...
        <div className="sf-auth-footer">
          <Link href="/konto/passwort-vergessen" style={{ fontSize: '0.85rem', color: 'var(--sf-gold)' }} data-testid="forgot-password-link">{t('forgot_password')}</Link>
        </div>
        <div className="sf-auth-footer">
          {t('auth_no_account')} <Link href="/konto/registrieren" data-testid="register-link">{t('auth_register_now')}</Link>
        </div>
      </div>
    </div>
  );
