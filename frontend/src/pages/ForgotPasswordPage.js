      ...
      <div className={variant === 'admin' ? 'adm-login-card' : 'sf-auth-card'}>
        ...
        {sent ? (
          <div style={{ padding: '1.5rem 0', textAlign: 'center' }}>
            ...
            <Link to={loginPath} className={variant === 'admin' ? 'adm-login-btn' : 'sf-auth-btn'} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', marginTop: '1.5rem', textDecoration: 'none' }} data-testid="back-to-login-btn">
          ...
          <>
            ...
            <div className={variant === 'admin' ? '' : 'sf-auth-footer'} style={{ textAlign: 'center', marginTop: '1rem' }}>
              <Link to={loginPath} style={{ color: 'var(--sf-gold)', fontSize: '0.85rem' }} data-testid="back-to-login-link">{t('forgot_back')}</Link>
            </div>
          </>
        )}
      </div>
