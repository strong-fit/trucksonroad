...
export default function AdminReviews() {
  ...
  const avgRating = reviews.filter(r => r.is_active).length > 0
    ? (reviews.filter(r => r.is_active).reduce((s, r) => s + r.rating, 0) / reviews.filter(r => r.is_active).length).toFixed(1)
    : '0.0';
  const activeCount = reviews.filter(r => r.is_active).length;

  const resetForm = () => {
    setForm({ author: '', rating: 5, text: '', date: new Date().toISOString().slice(0, 10), event_type: '', is_active: true });
    setEditing(null);
  };

  const startEdit = (r) => {
    setForm({ author: r.author, rating: r.rating, text: r.text, date: r.date, event_type: r.event_type || '', is_active: r.is_active });
    setEditing(r.id);
  };
  ...
  return (
    <AdminLayout title={t('admin_reviews')}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '1rem', marginBottom: '1.25rem' }}>
        ...
        <div className="adm-detail" style={{ padding: '1rem', textAlign: 'center' }} data-testid="reviews-active">
          <div style={{ fontSize: '1.6rem', fontWeight: 700 }}>{activeCount}</div>
          <div style={{ fontSize: '0.78rem', color: 'var(--adm-text-secondary)' }}>Aktiv / Sichtbar</div>
        </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.25rem', alignItems: 'start' }}>
        ...
        <div className="adm-detail" data-testid="review-form">
          <div className="adm-detail-header" style={{ borderBottom: '1px solid var(--adm-border)', paddingBottom: '0.75rem', marginBottom: '1rem' }}>
            <span className="adm-detail-title">{editing ? 'Bewertung bearbeiten' : 'Neue Bewertung'}</span>
            {editing && (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem' }}>
            <div>
              ...
              <input className="adm-input" value={form.author} onChange={e => setForm(f => ({ ...f, author: e.target.value }))} placeholder="z.B. Muster AG" data-testid="review-author" />
            </div>
            <div>
            ...
            <div>
              ...
              <textarea className="adm-input" rows={3} value={form.text} onChange={e => setForm(f => ({ ...f, text: e.target.value }))} placeholder="Was hat dem Kunden gefallen?" data-testid="review-text" />
            </div>
