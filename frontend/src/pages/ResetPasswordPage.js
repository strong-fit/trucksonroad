      ...
      <div className={variant === 'admin' ? 'adm-login' : 'sf-auth-page'} data-testid="reset-password-page">
        <div className={variant === 'admin' ? 'adm-login-card' : 'sf-auth-card'}>
          ...
          <div style={{ textAlign: 'center' }}>
            <Link to={loginPath} style={{ color: 'var(--sf-gold)' }}>{t('forgot_back')}</Link>
          </div>
        </div>
      </div>
    );
      ...
      <div className={variant === 'admin' ? 'adm-login-card' : 'sf-auth-card'}>
        ...
        {success ? (
          <div style={{ padding: '1.5rem 0', textAlign: 'center' }}>
            ...
            <Link to={loginPath} className={variant === 'admin' ? 'adm-login-btn' : 'sf-auth-btn'} style={{ display: 'inline-flex', alignItems: 'center', gap: '0.4rem', marginTop: '1.5rem', textDecoration: 'none', justifyContent: 'center' }} data-testid="go-login-btn">
              {t('auth_login')}
            </Link>
          </div>
        ) : (
